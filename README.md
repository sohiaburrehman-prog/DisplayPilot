...
- **Transactional display scenes** — capture primary display, resolution, refresh, position, rotation, and HDR across the active desktop; preview changes and automatically roll back if an apply fails. Windows display scaling is intentionally left unchanged.
- **Per-app / per-game auto-swap profiles** — when a chosen process starts, make a monitor primary or apply a complete saved scene; priorities and a selectable conflict rule decide which profile wins when apps overlap, and the original scene is restored after the stack exits.
- **Explainable, richer profile matching** — optionally require an exact executable path or window-title substring, then inspect why each profile matched and why the current winner controls the display.
- **Headless CLI** — list monitors/profiles/scenes; capture, rename, delete, export, import, preview, or apply scenes; explain profile conflicts; and set primary/HDR/projection. Add `--dry-run`, `--temporary`, and `--json` for stable scripting (`DisplayPilot.exe --help`).
...
DisplayPilot.exe --apply-profile <name-or-id> --dry-run --json
DisplayPilot.exe --list-scenes --json
DisplayPilot.exe --preview-scene <name-or-id> --json
DisplayPilot.exe --apply-scene <name-or-id> --json
DisplayPilot.exe --explain-profiles --json
